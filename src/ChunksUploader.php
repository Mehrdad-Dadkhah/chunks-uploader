...
class ChunksUploader
{
    ...
    public function finishUpload()
    {
        ...
        if (file_exists($targetPath) && $this->getUploadDirectory() != $this->getTempDirectory()) {
            $finalTargetPath = $this->getUploadDirectory() . DIRECTORY_SEPARATOR . $this->getNativeFileName();
        ...
        $response = [
            'status'                 => true,
            'uplodedName'            => $this->getNativeFileName(),
    ...
    private function getNativeFileName(): string
    {
        $uploadName = $this->getUploadName();
        $mimeType   = $this->getVideoMimeType();

        $fileInfo = new \SplFileInfo($uploadName);
        $fileName = explode('.' . $fileInfo->getExtension(), $fileInfo->getFilename())[0];

        return $fileName . $this->allMimeTypes[$mimeType];
    }
    ...
    private function setVideoMimeType(string $videoPath)
    {
        $this->videoMimeType = mime_content_type($videoPath);

        return $this;
    }

    private function getVideoMimeType(string $videoPath = null): string
    {
        if ($this->videoMimeType == null) {
            if ($videoPath == null) {
                throw new \Exception("Error: video path to get mime type is null!", 1);
            }

            $this->setVideoMimeType($videoPath);
        }
    ...
    private function checkMimeType(string $videoPath): bool
    {
        ...
        if (!in_array($videoMimeType, $this->allowedMimeTypes)) {
            return false;
        }
